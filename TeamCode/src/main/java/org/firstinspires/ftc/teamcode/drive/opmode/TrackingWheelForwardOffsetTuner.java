...
public class TrackingWheelForwardOffsetTuner extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (!(ppb.getLocalizer() instanceof StandardTrackingWheelLocalizer)) {
        ...
        for (int i = 0; i < NUM_TRIALS; i++) {
            ...
            ppb.turnAsync(Math.toRadians(ANGLE));

            while (!isStopRequested() && ppb.isBusy()) {
                double heading = ppb.getPoseEstimate().getHeading();
            ...
            double forwardOffset = StandardTrackingWheelLocalizer.FORWARD_OFFSET +
                    ppb.getPoseEstimate().getY() / headingAccumulator;
